...
final class Merchant_First_Admin {
	...
	public function styles() {
		...
		echo '<style id="mfa-styles">
			...
			/* Wholesale For WooCommerce ships a global `svg { max-width:100%;
			 * height:auto }` style reset, which resized the wordmark until it
			 * wrapped onto a second line and overflowed the 32px bar. Pin it
			 * hard, and keep the label on one line. Flex is avoided here: a
			 * plugin resetting display on .ab-item would let the mark wrap
			 * again, where white-space cannot be undone as easily. */
			#wp-admin-bar-mfa-toggle .ab-item { white-space: nowrap !important; }
			#wp-admin-bar-mfa-toggle .mfa-woo {
